...
def eval_detect_mAP(gt_dict, res_dict, minoverlap=0.5):
	...
	#print(name)
        ...
        for idx in range(pred_labels.shape[0]):
            ...
            #print(pred_labels[idx,3])
            
    # loading the detection result
    
    '''
    a={}
    a['BB']=np.array(res[cls]['bbox'])
    a['score']=np.array(res[cls]['score'])
    a['imgs']=np.array(res[cls]['img'])
    io.savemat('ddd',a)
    '''
    metrics = {}
    res_map = []
    ...
    for cls in classes:
        ...
        #save recall and prec
        '''
    	a={}
    	a['rec']=np.array(rec)
    	a['prec']=np.array(prec)
    	io.savemat(str(cls),a)
        '''
